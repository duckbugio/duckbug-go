...
// ingest: a payload that arrives without an "eventId" gets a fresh one minted
// per request, and a retry then stores the event twice. Nothing reaches this
// transport in that state - Duck sets a UUIDv4 on every event it builds, and
// ensureEventID in provider.go fills the field for anything else, including a
// caller driving CaptureEvent directly. The server validates it as uuid4, so a
// non-UUID idempotency key is rejected with 400 rather than honoured.
//
