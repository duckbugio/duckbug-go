...
- Go `1.25.13+`. The `go` directive in `go.mod` is the source of truth; this line is a copy for readers and loses to it if the two ever disagree. The floor is a patched release of the 1.25 line: earlier 1.25 patches carry advisories in `crypto/tls`, `encoding/asn1`, `net/url` and `net/http` that this module's HTTP transport reaches.
- Every change is validated on that minimum and on the current Go release. The exact versions are the matrix in the CI workflow under `.github/workflows/`, and are not repeated here.
...
- Delivery is retried on transport errors, `408`, `429` and every `5xx`, including status codes this SDK has never been taught about - those are what an edge in front of the installation invents, and dropping the event on the first one would lose it silently. `501 Not Implemented` is the single exception and is final, because it means the capability is not configured in that DuckBug installation and repeating the request cannot change that. Everything else is decided after one attempt. `409 Conflict` is not a failure: it means the backend already holds this `eventId`, so the result is marked `Duplicate`, `TransportResult.IsSuccess()` reports true and the transport failure handler is **not** called. Every other `4xx` is reported to the transport failure handler after one attempt.
- The retry predicate is identical in `duckbug-go`, `duckbug-js` and `duckbug-php`. What a duplicate looks like afterwards is not: `duckbug-js` also treats `409` as success, while `duckbug-php` hands it to its failure handler because its `isSuccess()` requires `2xx`. All three agree on not retrying it.
- Retries are opt-in here: `WithMaxRetries` defaults to `0`, because this transport can run inline in the caller's request path. `duckbug-js` and `duckbug-php` default to `2`.
